...
class AuditLogDataSourceTest {
    ...
    fun `fetch() should return parsed audit-logs when there is audit-logs returned by audit-log csv-parser`() {
        ...
        val auditLogs = listOf(
            AuditLog(UUID.randomUUID(), "Add", AuditType.PROJECT, timestamp, projectId, userId),
            AuditLog(UUID.randomUUID(), "Edit", AuditType.PROJECT, timestamp, projectId, userId),
            AuditLog(UUID.randomUUID(), "Delete", AuditType.PROJECT, timestamp, projectId, userId),
            AuditLog(UUID.randomUUID(), "Change State", AuditType.TASK, timestamp, taskId, userId),
        )
